...
class Solution:
    def longestBalanced(self, nums: List[int]) -> int:
        occs = defaultdict(deque)
        def sgn(x):return 1 if (x%2==0) else -1
        l = 0
        pref = [0]*len(nums)
        pref[0] = sgn(nums[0])
        occs[nums[0]].append(1)
        for i in range(1,len(nums)):
            ...
            if not occ:
                pref[i]+=sgn(nums[i])
            occ.append(i+1)
        seg = SegmentTree(pref)
        for i in range(len(nums)):
            l = max(l,seg.find_last(i+l,0)-i)
            nxt = len(nums)+1
            occs[nums[i]].popleft()
            if occs[nums[i]]:
                nxt = occs[nums[i]][0]
            seg.add(i+1,nxt-1,-sgn(nums[i]))
        return l